...
- Every note keyboard offers Markdown and plain-text export. The callback performs
...
- Migrations `phase5_user_preference_contract`,
  ...
  `telegram-webhook` version 25 and `process-job` version 26 are active.
- Type-check passes and the hermetic suite reports **508 passed, 0 failed**.
...
Tap both export buttons on a newly generated or reopened note and verify the
downloaded Markdown and text files. Before promoting beyond development,
